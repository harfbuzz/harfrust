...
use crate::hb::aat::layout::hb_aat_layout_remove_deleted_glyphs;
...
fn hb_ot_substitute_plan(ctx: &mut hb_ot_shape_context_t) {
    ...
    if ctx.plan.apply_morx {
        aat::layout::hb_aat_layout_substitute(ctx.plan, ctx.face, ctx.buffer, ctx.features);
    } else {
...
fn position_complex(ctx: &mut hb_ot_shape_context_t) {
    ...
    if ctx.plan.apply_morx {
        aat::layout::hb_aat_layout_zero_width_deleted_glyphs(ctx.buffer);
    }
...
fn position_by_plan(plan: &hb_ot_shape_plan_t, face: &hb_font_t, buffer: &mut hb_buffer_t) {
    if plan.apply_gpos {
        ...
        aat::layout::hb_aat_layout_position(plan, face, buffer);
    }
    if plan.apply_kern {
    ...
    if plan.apply_trak {
        aat::layout::hb_aat_layout_track(plan, face, buffer);
    }
}
