...
impl<'a> OtTables<'a> {
    pub fn new(
        ...
        let gsub_len = gsub
            ...
            .unwrap_or(0);
        let gsub = gsub.map(|table| GsubTable {
            table,
            lookups: &cache.gsub,
        });
        ...
        let gpos_len = gpos
            ...
            .map(|t| t.offset_data().len() as u32)
            .unwrap_or(0);
        let gpos = gpos.map(|table| GposTable {
            table,
            lookups: &cache.gpos,
        });
        ...
        let gdef_len = font
            ...
            .map(|t| t.offset_data().len() as u32)
            .unwrap_or(0);
        let gdef = if is_gdef_blocklisted(gdef_len, gsub_len, gpos_len) {
            GdefTable::default()
        } else {
            GdefTable::new(font)
        };
