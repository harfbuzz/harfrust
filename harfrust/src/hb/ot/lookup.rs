...
use read_fonts::{
    ...
    FastRead, FontData, FontRead, Offset, ReadError,
};
        ...
        for subtable_info in &self.subtables {
            ...
            let result = match subtable_info.kind {
                SubtableKind::SingleSubst1 => {
                    SingleSubstFormat1::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::SingleSubst2 => {
                    SingleSubstFormat2::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::MultipleSubst1 => {
                    MultipleSubstFormat1::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::AlternateSubst1 => {
                    AlternateSubstFormat1::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::LigatureSubst1 => {
                    LigatureSubstFormat1::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::ReverseChainContext => {
                    ReverseChainSingleSubstFormat1::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::ContextFormat1 => {
                    SequenceContextFormat1::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::ContextFormat2 => {
                    SequenceContextFormat2::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::ContextFormat3 => {
                    SequenceContextFormat3::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::ChainedContextFormat1 => {
                    ChainedSequenceContextFormat1::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::ChainedContextFormat2 => {
                    ChainedSequenceContextFormat2::fast_read(data, ()).would_apply(ctx)
                }
                SubtableKind::ChainedContextFormat3 => {
                    ChainedSequenceContextFormat3::fast_read(data, ()).would_apply(ctx)
                }
                _ => continue,
            };
            if result {
...
macro_rules! apply_fns {
    ($apply:ident, $apply_cached:ident, $ty:ident) => {
        fn $apply(
            ...
            let t = $ty::fast_read(FontData::new(table_data), ());
...
impl SubtableInfo {
    fn new(
        ...
        let (kind, (external_cache, cache_cost, coverage), apply_fns): (
            ...
            (true, 1) => match SingleSubst::fast_read(data, ()) {
            ...
            (false, 1) => match SinglePos::fast_read(data, ()) {
                SinglePos::Format1(s) => (
            ...
            (true, 2) => {
                let t = MultipleSubstFormat1::fast_read(data, ());
                (
                    SubtableKind::MultipleSubst1,
            ...
            (false, 2) => match PairPos::fast_read(data, ()) {
                PairPos::Format1(s) => (
            ...
            (true, 3) => {
                let t = AlternateSubstFormat1::fast_read(data, ());
                (
                    SubtableKind::AlternateSubst1,
                    (maybe_external_cache(&t), t.cache_cost(), t.coverage().ok()?),
            ...
            (false, 3) => {
                let t = CursivePosFormat1::fast_read(data, ());
                (
            ...
            (true, 4) => {
                let t = LigatureSubstFormat1::fast_read(data, ());
                (
                    SubtableKind::LigatureSubst1,
                    (maybe_external_cache(&t), t.cache_cost(), t.coverage().ok()?),
            ...
            (false, 4) => {
                let t = MarkBasePosFormat1::fast_read(data, ());
                (
            ...
            (true, 5) | (false, 7) => match SequenceContext::fast_read(data, ()) {
            ...
            (false, 5) => {
                let t = MarkLigPosFormat1::fast_read(data, ());
                (
            ...
            (true, 6) | (false, 8) => match ChainedSequenceContext::fast_read(data, ()) {
            ...
            (false, 6) => {
                let t = MarkMarkPosFormat1::fast_read(data, ());
                (
            ...
            (true, 8) => {
                let t = ReverseChainSingleSubstFormat1::fast_read(data, ());
                (
                    SubtableKind::ReverseChainContext,
                    (maybe_external_cache(&t), t.cache_cost(), t.coverage().ok()?),
